...
class FileParser {
	 ...
	 *
	 * @param {Object} `opts` - Module options/settings.
	 * @return {String}
	 */
	parse( opts ) {
		let output = '';
		...
		FILES.forEach( ( file ) => {
			...
			obj.meta = file.meta || {};
			obj.matches = this.getInlineNotes( file.data, opts );
	...
	addFile( file ) {
		if ( !file ) {
			throw new Error( 'Please ensure that `addFile()` is invoked with a valud `file` argument.' );
		}
	 ...
	 * @return {Object}
	 */
	getInlineNotes( file, opts ) {
		let output = {};
		let prefixCaptureGroup = this.getPrefixCaptureGroup( opts.prefixes );

		// Check `input` for text matching each of the `keywords`.
		opts.keywords.forEach( ( keyword ) => {
	...
	escapeSpecialChars( str ) {
		...
		return str.split( '' )
			.map( ( char ) => {
				return ( config.specialChars.includes( char ) ) ? '\\' + char : char; // NOTE - Extra '\' character required in order to escape... the escape...
			} )
			.join( '' );
	}
}
